...
>, ServiceError> {
  ...
  let (rpc_extensions_builder, rpc_setup) = {
    ...
    let finality_proof_provider = GrandpaFinalityProofProvider::new_for_service(backend.clone(), Some(shared_authority_set));
...
pub fn new_full_base(config: Configuration,
  ...
  let sc_service::PartialComponents {
    ...
    inherent_data_providers,
    other: (_, import_setup, rpc_setup),
  } = new_partial(&config)?;
...
pub fn new_light_base(config: Configuration) -> Result<(
  TaskManager, RpcHandlers, Option<TelemetryConnectionNotifier>,
  Arc<LightClient>,
  Arc<sc_network::NetworkService<Block, <Block as BlockT>::Hash>>,
